...
class Paletka:
    ...
    def __init__(self, paletka, pozycja):
        ...
        self.paletka.odbic = 0

    def add_bounce(self):
        self.paletka.odbic += 1
    ...
    def bounce(self):
        ...
        if abs(self.paletka.y - ball.y ) > 18:
                return False
        if self.paletka.x > ball.x and ball.direction_x == "left":
            ball.direction_x = "right"
        elif self.paletka.x < ball.x and ball.direction_x == "right":
...
def sprawdz_czy_odbijemy():
    if paletka_a.bounce():
        ...
        paletka_a.add_bounce()
    if paletka_b.bounce():
        ball.direction_y = "up"
        paletka_b.add_bounce()
...
ball = Actor("crystal-ball-2090496_640.png")
ball.x = randint(35, WIDTH -35)
ball.y = HEIGHT//2
...
def draw():
    screen.blit("the-road-815297_1280.jpg", (0,0))
    if not ball.start:
        screen.draw.text(
